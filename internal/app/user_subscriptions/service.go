...
func (us UserSubscriptionService) Get(query AllowedQuery) (error, []ViewModel) {
	...
	if err := database.DB.Table("user_subscriptions").Preload("Users").Preload("Subscriptions").Where(&query.DataQuery).Offset(offset).Limit(query.Limit).Find(&userSubs).Error; err != nil {
		return err, userSubs
	}
